function ps
    ...
    switch $type
        case t
            if test -z "$selected"
                set -f selected (cat "$XDG_CACHE_HOME/ps_proyects" | fzf | string replace -r '^~' "$HOME")
            end
            if test -n "$selected"
                cd "$selected"
                eval "$cmd"
        ...
        case s w
            if test -z "$selected"
                tmux new-window -t "$curr_session:0" -n sessionizer "cat '$XDG_CACHE_HOME/ps_proyects' | fzf | string replace -r '^~' '$HOME' > '$XDG_CACHE_HOME/ps_selected'"
